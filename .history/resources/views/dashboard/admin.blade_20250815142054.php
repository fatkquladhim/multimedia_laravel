...
@section('content')  
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm hover-scale animate-fade-in cursor-pointer">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-light-blue-600 dark:text-light-blue-400"><?php echo htmlspecialchars($anggota_count); ?></p>
                        </div>
            ...
            foreach ($divisi_list as $divisi) {
                ...
                // Link ke anggota.php dengan filter divisi
                $divisi_param = strtolower($divisi);
            ?>
                <a href="anggota/anggota.php?divisi=<?php echo urlencode($divisi_param); ?>" class="block">
                    <div class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm hover-scale animate-fade-in cursor-pointer">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm text-gray-600 dark:text-gray-400">Anggota <?php echo htmlspecialchars($divisi); ?></p>
                                <p class="text-2xl font-bold text-<?php echo $color; ?>-600 dark:text-<?php echo $color; ?>-400"><?php echo htmlspecialchars($count); ?></p>
                            </div>
                            <div class="w-12 h-12 bg-<?php echo $color; ?>-100 dark:bg-<?php echo $color; ?>-900 rounded-lg flex items-center justify-center">
                                <i class="<?php echo $icon; ?> text-<?php echo $color; ?>-600 dark:text-<?php echo $color; ?>-400"></i>
                            </div>
                        </div>
                    </div>
                </a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="space-y-2">
                            ...
                            <?php while ($row = $tugas_belum_dikerjakan->fetch_assoc()): ?>
                                <div class="flex items-center p-3 bg-light-blue-50 dark:bg-light-blue-900 rounded-xl hover-scale justify-between">
                                    <div>
                                        ...
                                        <span class="ml-2 text-sm text-gray-600 dark:text-gray-300">(<?php echo htmlspecialchars($row['username']); ?>)</span>
                                    ...
                                    <button class="ml-4 px-3 py-1 bg-light-blue-600 hover:bg-light-blue-700 text-white rounded text-xs font-medium transition-colors detail-tugas-btn"
                                        data-nama="<?php echo htmlspecialchars($row['nama_tugas']); ?>"
                        ...
                        <?php else: ?>
                            <div class="text-center py-8">
                                ...
                                <p class="text-gray-600 dark:text-gray-400">Tidak ada tugas yang belum dikerjakan saat ini.</p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm animate-fade-in flex flex-col h-full">
                ...
                <div class="flex-1 flex flex-col justify-between">
                    <div class="space-y-2">
                        <?php if ($tugas_selesai->num_rows > 0): ?>
                            <?php while ($row = $tugas_selesai->fetch_assoc()): ?>
                                <div class="flex items-center p-3 bg-green-50 dark:bg-green-900 rounded-xl hover-scale justify-between detail-tugas-selesai-btn"
                                    ...
                                    data-username="<?php echo htmlspecialchars($row['username']); ?>"
                                    data-status="Selesai"
                                    data-tanggal="<?php echo date('d M Y H:i', strtotime($row['tanggal_submit'])); ?>"
                                >
                                    <div>
                                        <span class="font-medium text-gray-800 dark:text-white">
                                            <?php echo htmlspecialchars($row['nama_tugas']); ?>
                                        ...
                                        <div class="text-xs text-gray-400 mt-1">
                                            Selesai: <?php echo date('d M Y H:i', strtotime($row['tanggal_submit'])); ?>
                                        </div>
                                    </div>
                                </div>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <div class="text-center py-8">
                                ...
                                <p class="text-gray-600 dark:text-gray-400">Tidak ada tugas yang sudah selesai.</p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>

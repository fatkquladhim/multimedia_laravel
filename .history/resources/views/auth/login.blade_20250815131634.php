...
<div class="min-h-screen bg-gradient-to-br from-blue-600 via-blue-500 to-blue-300 flex flex-col min-h-screen">
    ...
    <div class="flex flex-1 flex-col items-center justify-center">
        <main class="w-full max-w-xs sm:max-w-sm md:max-w-md p-2 sm:p-0" style="transform:scale(0.95);">
            <div class="flex justify-center mb-8">
                <div>
                    <img src="{{ asset('imagerev-removebg-preview.png') }}" style="max-width:80px;">
                </div>
            </div>
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="username" id="username" placeholder="Username" class="glass-input w-full pl-12 pr-4 py-4 rounded-full text-white placeholder-blue-100 focus:outline-none transition-all duration-200" required />
                </div>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <svg class="w-5 h-5 text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input type="password" name="password" id="password" placeholder="Password" class="glass-input w-full pl-12 pr-12 py-4 rounded-full text-white placeholder-blue-100 focus:outline-none transition-all duration-200" required />
                ...
                <div class="flex items-center justify-between text-sm">
                    ...
                    <a href="#" class="text-blue-100 hover:text-white transition-colors duration-200">Forgot Password?</a>
                        ...
                        <div class="bg-white rounded-lg shadow-lg w-full max-w-xs p-6 relative animate-fade-in">
                            ...
                            <ul id="usernameList" class="space-y-2 max-h-60 overflow-y-auto">
                                @foreach($usernames as $uname)
                                    <li><button type="button" class="select-username w-full text-left px-3 py-2 rounded hover:bg-blue-100 text-gray-700" data-username="{{ $uname }}">{{ $uname }}</button></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                </div>
                <button type="submit" class="login-btn w-full py-2 font-semibold rounded-full focus:outline-none glass-header px-6 py-4 text-white relative z-10">LOGIN</button>
            </form>
        </main>
    </div>
    <footer class="flex flex-col items-center justify-center mt-8 mb-4">
        ...
        <p class="text-blue-100 text-sm mt-1">Versi Web: {{ $version ?? '2.0.0' }}</p>
    </footer>
</div>

            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('izin-nugas.create') }}" class="btn btn-primary">Tambah Izin</a>
                    </div>
                </div>
                  <div class="card-body">
                    <div class="table-responsive">
                      <table id="multi-filter-select" class="display table table-striped table-hover">
                        ...
                        <tbody>
                           @foreach($izin_nugas as $key => $item)
                          <tr>
                            <td>{{ $key + 1 }}</td>
                            ...
                            <td>{{ $item->alasan }}</td>
                            <td>
                              <a href="{{ route('izin-nugas.show', $item->id) }}" class="btn btn-info btn-sm">Lihat</a>
                              <a href="{{ route('izin-nugas.edit', $item->id) }}" class="btn btn-warning btn-sm">Edit</a>
                              <form action="{{ route('izin-nugas.destroy', $item->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
